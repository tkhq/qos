...
use qos_p256::{
	...
	QosKeySet, QosKeySetV0Public, P256_SECRET_LEN,
};
...
fn provision_yubikey_works() {
	...
	// Check that public keys where written
	{
		...
		assert!(QosKeySetV0Public::from_bytes(&bytes).is_ok());
		QosKeySetV0Public::from_bytes(&bytes).unwrap();
	}
}
...
fn advanced_provision_yubikey_works() {
	...
	let pair = QosKeySet::from_hex_file(&*master_seed_path).unwrap();
