...
use qos_p256::{QuorumKey, QuorumKeyError, QuorumKeyPublic};
...
pub fn advanced_provision_yubikey<P: AsRef<Path>>(
	...
	let public_key_bytes = crate::yubikey::pair_public_key(&mut yubikey)?;
	let other = key_set.public_key().to_bytes();
...
pub(crate) fn boot_genesis<P: AsRef<Path>>(
	...
	let dr_key = if let Some(p) = dr_key_path {
		let public = QuorumKeyPublic::from_hex_file(p)
...
pub(crate) fn verify_genesis<P: AsRef<Path>>(
	...
	let versioned_secret_hex = fs::read_to_string(&master_seed_path)
	...
	if pair.to_hex() != versioned_secret_hex.as_bytes() {
		return Err(Error::ErrorReadingSeed);
	}

	// check quorum_key_hash
	if sha_512(versioned_secret_hex.as_bytes())
		!= genesis_output.quorum_key_hash
	{
...
pub(crate) fn generate_manifest<P: AsRef<Path>>(
	...
	let quorum_key = QuorumKeyPublic::from_hex_file(&quorum_key_path)
...
pub(crate) fn boot_standard<P: AsRef<Path>>(
	...
	if unsafe_skip_attestation {
		...
		QuorumKeyPublic::from_bytes(&eph_pub_bytes)
...
pub(crate) fn p256_verify<P: AsRef<Path>>(
	...
	let public = QuorumKeyPublic::from_hex_file(pub_path)?;
	let signature_bytes = {
...
pub(crate) fn get_ephemeral_key_hex<P: AsRef<Path>>(
	...
	let ephemeral_key = QuorumKeyPublic::from_bytes(
		&attestation_doc
			.public_key
			.expect("No ephemeral key in the attestation doc"),
	)
...
pub(crate) fn dangerous_dev_boot<P: AsRef<Path>>(
	...
	let shares =
		qos_crypto::shamir::shares_generate(quorum_pair.as_bytes(), 2, 2)
			.unwrap();
	assert_eq!(
		shares.len(),
		2,
...
fn get_share_set<P: AsRef<Path>>(dir: P) -> ShareSet {
	let mut members: Vec<_> = find_file_paths(&dir)
		...
		.filter_map(|path| {
			...
			let public = QuorumKeyPublic::from_hex_file(path)
...
mod tests {
	...
	fn setup() -> Setup {
		...
		let quorum_key: QuorumKeyPublic =
