...
// We don't use DR for preprod; the keys are checked in and hold no value.
const DR_KEY_PUBLIC_PATH: &str = "./mock/mock_p256_dr.pub";
const DR_KEY_PRIVATE_PATH: &str = "./mock/mock_p256_dr.secret.keep";

/// This test is also a utility script to generate new quorum keys encrypted to existing "dev" operator in our preprod environment.
/// Useful when we create new enclave applications.
#[tokio::test]
async fn preprod_genesis() {
	let host_port = qos_test_primitives::find_free_port().unwrap();
	let tmp = Path::new("/tmp/preprod-genesis");
	if tmp.exists() {
	...
	let usock = tmp_dir("preprod_genesis.sock");
	let secret_path = tmp_dir("preprod_genesis.secret");
	let pivot_path = tmp_dir("preprod_genesis.pivot");
	...
	for dev_user in [dev1, dev2] {
		fs::copy(
			Path::new(&preprod_fixture_dev_pub(&dev_user)),
	...
	let mut decrypted_shares: Vec<_> = genesis_output
		...
		.map(|member| {
			...
			let share_key_path = preprod_fixture_dev_secret(&alias);
	...
	for user in [&dev1, &dev2] {
		...
		let secret_path = preprod_fixture_dev_secret(&user);
		assert!(Command::new("../target/debug/qos_client")
			.args([
				"after-genesis",
				"--secret-path",
