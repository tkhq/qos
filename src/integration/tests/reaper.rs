...
use qos_core::{
	...
	protocol::services::boot::{
		manifest::v2, BridgeConfig, ManifestEnvelopeV1, ManifestVersion,
...
async fn reaper_injects_v2_manifest_env_and_clears_host_env() {
	...
	drop(fs::remove_file(&*secret_path));
	drop(fs::remove_file(PIVOT_OK2_SUCCESS_FILE));
	...
	let manifest_envelope = v2::ManifestEnvelopeV2 {
		manifest: v2::ManifestV2 {
			...
			manifest_set: Default::default(),
			share_set: Default::default(),
			enclave: Default::default(),
...
async fn reaper_handles_non_zero_exits() {
	...
	// Make sure we have written everything necessary to pivot, except the
	// quorum key
	handles.put_manifest_envelope(ManifestEnvelopeV1::default()).unwrap();
	assert!(handles.pivot_exists());
