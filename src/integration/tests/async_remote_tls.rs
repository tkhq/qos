...
async fn fetch_async_remote_tls_content() {
	...
	let enclave_socket_path = format!("{REMOTE_TLS_TEST_ENCLAVE_SOCKET}_0"); // TODO: ales - unhack

	let _enclave_app: ChildWrapper = Command::new(PIVOT_ASYNC_REMOTE_TLS_PATH)
		.arg(&enclave_socket_path)
		.arg(REMOTE_TLS_TEST_NET_PROXY_SOCKET)
		.spawn()
		.unwrap()
		.into();

	let enclave_pool = AsyncStreamPool::connect(
		std::iter::once(SocketAddress::new_unix(&enclave_socket_path)),
