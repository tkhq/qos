	...
	async fn process(&self, request: Vec<u8>) -> Vec<u8> {
		...
		match msg {
			PivotRemoteTlsMsg::RemoteTlsRequest { host, path } => {
				...
				let mut tls =
					conn.connect(server_name, &mut stream).await.unwrap();
...
async fn main() {
	...
	eprintln!("socket_path: {}\nproxy_path: {}", socket_path, proxy_path);

	let pool = AsyncStreamPool::new(std::iter::once(SocketAddress::new_unix(
		socket_path,
	)));

	let proxy_pool = AsyncStreamPool::connect(
