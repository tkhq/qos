...
//!
//! Design notes:
//! - **Provenance tracking** (wasmlanche Pattern #1): every `alloc` records
//!   `(ptr, len)` in `ALLOCATIONS`. `take_input` recovers the length from
//!   the map rather than trusting the host-supplied `len` blindly, so a
//!   malformed host call can't induce out-of-bounds reads of guest memory.
//! - **Universal host-to-guest passing** (wasmlanche Pattern #4): host
//!   always allocates via the exported `alloc` and writes bytes there;
//!   guest reads via `take_input(ptr, host_len)`.
...
///
/// Verifies that `ptr` was previously returned by [`alloc`] (provenance check)
/// and that the host-supplied length matches what we recorded. The returned
/// `Vec<u8>` owns the memory; the entry is removed from `ALLOCATIONS`.
...
pub fn install_panic_hook() {
	...
	// instance, which the host translates into an error.
}
