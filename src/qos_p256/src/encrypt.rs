...
use crate::{bytes_os_rng, QosKeySetError, PUB_KEY_LEN_UNCOMPRESSED};
...
impl P256EncryptPair {
	...
	pub fn decrypt(
		...
		let Envelope {
			...
			.map_err(|_| QosKeySetError::FailedToDeserializeEnvelope)?;

		let nonce = Nonce::from_slice(&nonce);
		let ephemeral_sender_public =
			PublicKey::from_sec1_bytes(&ephemeral_sender_public_bytes)
				.map_err(|_| QosKeySetError::FailedToDeserializePublicKey)?;
		...
		cipher
			...
			.map_err(|_| QosKeySetError::AesGcm256DecryptError)
	}
	...
	pub fn encrypt(&self, message: &[u8]) -> Result<Vec<u8>, QosKeySetError> {
		...
		let ephemeral_sender_public: [u8; PUB_KEY_LEN_UNCOMPRESSED as usize] =
			ephemeral_sender_private
				...
				.try_into()
				.map_err(|_| {
					QosKeySetError::FailedToCoercePublicKeyToIntendedLength
				})?;
		...
		let encrypted_message = cipher
			...
			.map_err(|_| QosKeySetError::AesGcm256EncryptError)?;
		...
		borsh::to_vec(&envelope)
			.map_err(|_| QosKeySetError::FailedToSerializeEnvelope)
	}
...
fn create_cipher(
...
) -> Result<Aes256Gcm, QosKeySetError> {
	...
	Aes256Gcm::new_from_slice(&shared_key[..AES256_KEY_LEN])
		.map_err(|_| QosKeySetError::FailedToCreateAes256GcmCipher)
}
...
///
/// Note: lengths are appended after each field rather than prepended. This
/// differs from NIST SP 800-56Ar3 5.8.2 (<https://nvlpubs.nist.gov/nistpubs/SpecialPublications/NIST.SP.800-56Ar3.pdf>)
/// which specifies `len || data`. The ordering is a historical artifact.
fn create_additional_associated_data(
	...
	let ephemeral_sender_len_int: u8 = ephemeral_sender_public
		...
		.len()
		.try_into()
		.map_err(|_| QosKeySetError::CannotCoerceLenToU8)?;
...
impl AesGcm256Secret {
	...
	/// happen in practice.
	pub fn encrypt(&self, msg: &[u8]) -> Result<Vec<u8>, QosKeySetError> {
		let nonce = {
...
mod test_asymmetric {
	...
	fn tampered_ephemeral_sender_key_errors() {
		...
		assert_eq!(
			alice_pair.decrypt(&tampered_envelope).unwrap_err(),
			QosKeySetError::FailedToDeserializePublicKey
		);
	}
	...
	fn tampered_envelope_errors() {
		...
		assert_eq!(
			...
			QosKeySetError::FailedToDeserializeEnvelope
		);
	}
