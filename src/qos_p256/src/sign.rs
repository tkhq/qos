...
use crate::{QuorumKeyError, PUB_KEY_LEN_UNCOMPRESSED};
...
impl P256SignPublic {
	...
	pub fn verify(
		...
		let signature = Signature::try_from(signature)
			.map_err(|_| QuorumKeyError::FailedToDeserializeSignature)?;

		self.public
			.verify(message, &signature)
			.map_err(|_| QuorumKeyError::FailedSignatureVerification)
	}
