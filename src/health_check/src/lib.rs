...
use generated::grpc::health::v1::{
	...
	health_server::{
		Health as K8HealthService, HealthServer as K8HealthServer,
	},
	HealthCheckRequest as K8HealthCheckRequest,
	HealthCheckResponse as K8HealthCheckResponse,
};
use generated::health::health_check_service_server::{
	HealthCheckService, HealthCheckServiceServer,
};
use generated::health::{
	AppHealthRequest, AppHealthResponse, EnclaveHealthRequest,
	EnclaveHealthResponse, HostHealthRequest, HostHealthResponse,
};
use generated::tonic;
use qos_core::{
	client::Client as SocketClient,
	io::SocketAddress,
	protocol::{msg::ProtocolMsg, ProtocolPhase},
};
	...
	T: AppHealthCheckable + Send + Sync + 'static,
{
	...
	fn k8_request(
		...
		K8HealthCheckResponse { status: status as i32 }
	}
}

type ResponseStream = Pin<
	Box<dyn Stream<Item = Result<K8HealthCheckResponse, tonic::Status>> + Send>,
>;
	...
	T: AppHealthCheckable + Send + Sync + 'static,
{
	async fn check(
		...
		tonic::Response<K8HealthCheckResponse>,
		tonic::Status,
	> {
	...
	) -> std::result::Result<tonic::Response<Self::WatchStream>, tonic::Status>
	{
		...
		Ok(tonic::Response::new(Box::pin(output_stream) as Self::WatchStream))
	}
}
