...
use nitro_cli::{
    enclave_proc_comm::{
        ...
        enclave_proc_connect_to_single,
	},
    common::{
        json_output::{
            EnclaveDescribeInfo,
...
fn healthy() -> Result<(), Box<dyn std::error::Error>> {
	...
	let describe_args = DescribeEnclavesArgs {
        metadata: false,
    };
...
fn shutdown() {
    ...
    let enclave_name = std::env::var("ENCLAVE_NAME").unwrap_or("nitro".to_string());
	let enclave_id = get_id_by_name(enclave_name).or_else(|_| {
        Err("Failed to parse enclave name")
	}).unwrap();
    let mut comm = enclave_proc_connect_to_single(&enclave_id).or_else(|_| {
        Err("Failed to send command to Enclave")
	}).unwrap();
    ...
    let _ = enclave_proc_command_send_single::<EmptyArgs>(
		...
		Err("Unable to terminate Enclave")
    });
}
