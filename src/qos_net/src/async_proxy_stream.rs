...
impl AsyncWrite for AsyncProxyStream {
	fn poll_write(
...
impl AsyncRead for AsyncProxyStream {
	fn poll_read(
		mut self: std::pin::Pin<&mut Self>,
		cx: &mut std::task::Context<'_>,
...
fn send_proxy_req(
	...
	let written = Pin::new(&mut ps.stream).poll_write(cx, &bytes);
...
fn get_proxy_resp(
	...
	loop {
		let read = Pin::new(&mut ps.stream).poll_read(cx, &mut read_buf);
