...
QuorumOS uses Protocol Buffers (protobuf) for wire format encoding to enable cross-language interoperability and backwards compatible schema updates. This document describes the encoding conventions and provides guidance for working with proto types.
...
We intend to migrate to protobuf serialization for signing payload pre-images to enable backwards compatible signature verification.

## Deterministic Encoding

For cryptographic operations (hashing, signing), deterministic encoding is critical. QOS follows these rules:

1. **No map fields**: `map<>` fields are prohibited as their iteration order is undefined
2. **Optional fields**: Use `optional` for nullable fields
3. **Empty repeated fields**: Not serialized (protobuf default)
...
The `optional` keyword allows distinguishing between "not set" (`None`) and "explicitly set to empty" (`Some("")`). Use `optional` when this distinction matters for application logic.
