...
impl AsyncStream {
	...
	fn unix_accepted(stream: UnixStream) -> Self {
		Self { inner: InnerStream::Unix(stream), timeout: Duration::ZERO }
	}

	// accept a new connection, used by server side
	#[cfg(feature = "vm")]
	fn vsock_accepted(stream: VsockStream) -> Self {
		Self { inner: InnerStream::Vsock(stream), timeout: Duration::ZERO }
	}
	...
	pub async fn connect(
		...
		match addr {
			SocketAddress::Unix(uaddr) => {
				...
				let inner = retry_unix_connect(path, timeout).await?;

				Ok(Self { inner: InnerStream::Unix(inner), timeout })
			}
			#[cfg(feature = "vm")]
			SocketAddress::Vsock(vaddr) => {
				let inner =
					retry_vsock_connect(vaddr.cid(), vaddr.port(), timeout)
						.await?;

				Ok(Self { inner: InnerStream::Vsock(inner), timeout })
			}
		}
	}
