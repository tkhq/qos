...
/// Helper type to wrap AsyncStreamPool in Arc and Mutex. Used to allow multiple processors to run across IO
/// await points without locking the whole set.
pub type SharedAsyncStreamPool = Arc<Mutex<AsyncStreamPool>>;
...
impl AsyncStreamPool {
	...
	pub fn to_shared(self) -> SharedAsyncStreamPool {
