...
use super::Approval;
use crate::{client::ClientError, io::IOError, protocol::ProtocolPhase};
	...
	fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
		match self {
			...
			Self::ReconstructionErrorIncorrectPubKey => {
				write!(
					f,
					"reconstructed incorrect public key while provisioning"
				)
			}
			...
			Self::InvalidPrivateKey => {
				write!(f, "could not create private key")
			}
			...
			Self::FailedToPutEphemeralKey => {
				write!(f, "failed to write ephemeral key")
			}
			...
			Self::FailedToPutQuorumKey => {
				write!(f, "failed to write quorum key")
			}
			...
			Self::FailedToPutPivot => {
				write!(f, "failed to write pivot executable")
			}
