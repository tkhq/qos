...
pub use qos_proto::{GenesisMemberOutput, GenesisOutput, GenesisSet, QuorumMember};
...
pub(in crate::protocol) fn boot_genesis(
	...
	let member_outputs: Result<Vec<_>, _> = zip(shares, genesis_set.members.iter().cloned())
		.map(|(share, share_set_member)| -> Result<GenesisMemberOutput, ProtocolError> {
			...
			Ok(GenesisMemberOutput {
				share_set_member: Some(share_set_member),
	...
	let genesis_output = GenesisOutput {
		...
		quorum_key_hash: sha_512(hex_master_seed.as_bytes()).to_vec(),
	...
	let nsm_response = {
		let request = NsmRequest::Attestation {
			user_data: Some(genesis_output.proto_hash().to_vec()),
			nonce: None,
			public_key: None,
		};
	...
	fn boot_genesis_works() {
		...
		let shares: Vec<Vec<u8>> = zipped
			.map(|(output, pair)| {
				...
				assert_eq!(sha_512(decrypted_share).to_vec(), output.share_hash);
		...
		assert_eq!(quorum_key_hash.to_vec(), output.quorum_key_hash);
	}
}
