...
fn shard(
	...
	let shard_config = state.shard_config.ok_or(ProtocolError::NotShardBooted)?;
...
pub(in crate::protocol) fn shard_provision(
	...
	if public_key_bytes != shard_config.expected_quorum_key {
		return Err(ProtocolError::WrongQuorumKey);
	}

	shard(state, shard_config, pair)?;
