	...
	fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
		match self {
			...
			Self::ReconstructionErrorIncorrectPubKey => {
				write!(f, "reconstructed incorrect public key while provisioning")
			}
			...
			Self::NotEnoughApprovals => {
				write!(f, "manifest envelope did not have enough approvals")
			}
			Self::NoMatchingRoute(phase) => {
				write!(f, "no matching route for phase {:?}", phase)
			}
			Self::InvalidPivotHash { expected, actual } => {
				write!(f, "invalid pivot hash: expected {expected}, got {actual}")
			}
			Self::OversizeMsg => write!(f, "message is too large"),
			Self::InvalidMsg => write!(f, "message could not be deserialized"),
			Self::EnclaveClient => write!(f, "enclave client error"),
			Self::DecryptionFailed => write!(f, "decryption failed"),
			Self::InvalidPrivateKey => write!(f, "could not create private key"),
			...
			Self::CannotModifyPostPivotStatic => {
				write!(f, "cannot modify state after pivoting")
			}
			Self::FailedToGetEphemeralKey(e) => {
				write!(f, "failed to get ephemeral key: {:?}", e)
			}
			Self::FailedToPutEphemeralKey => write!(f, "failed to write ephemeral key"),
			Self::CannotRotateNonExistentEphemeralKey => {
				write!(f, "cannot rotate non-existent ephemeral key")
			}
			Self::CannotDeleteEphemeralKey(msg) => {
				write!(f, "cannot delete ephemeral key: {}", msg)
			}
			Self::FailedToGetQuorumKey(e) => {
				write!(f, "failed to get quorum key: {:?}", e)
			}
			Self::FailedToPutQuorumKey => write!(f, "failed to write quorum key"),
			Self::FailedToGetManifestEnvelope => {
				write!(f, "failed to get manifest envelope")
			}
			Self::FailedToPutManifestEnvelope => {
				write!(f, "failed to write manifest envelope")
			}
			Self::FailedToPutPivot => write!(f, "failed to write pivot executable"),
			Self::AppClientRecvTimeout => {
				write!(f, "app client receive timed out")
			}
			Self::AppClientRecvInterrupted => {
				write!(f, "app client receive was interrupted")
			}
			Self::AppClientRecvConnectionClosed => {
				write!(f, "app client receive connection closed")
			}
			Self::AppClientConnectError(msg) => {
				write!(f, "app client connect error: {}", msg)
			}
			Self::AppClientSendError(msg) => {
				write!(f, "app client send error: {}", msg)
			}
			Self::AppClientError(msg) => {
				write!(f, "app client error: {}", msg)
			}
			Self::OversizedPayload => write!(f, "payload is too large"),
			Self::ProtocolMsgDeserialization => {
				write!(f, "protocol message could not be deserialized")
			}
			Self::BadShareSetApprovals => {
				write!(f, "unexpected share set approvals in manifest envelope")
			}
			...
			Self::IncorrectSecretLen => {
				write!(f, "provisioned secret has incorrect length")
			}
			Self::QosAttestError(msg) => {
			...
			Self::MissingEphemeralKey => {
				write!(f, "attestation document missing ephemeral key")
			}
			Self::InvalidEphemeralKey => {
				write!(f, "ephemeral key cannot be decoded")
			}
			Self::InvalidEncryptedQuorumKeySignature => {
				write!(f, "invalid signature over encrypted quorum key")
			}
			Self::EncryptedQuorumKeyInvalidLen => {
				write!(f, "invalid length for encrypted quorum key")
			}
			Self::InvalidQuorumSecret => write!(f, "invalid quorum secret"),
			Self::WrongQuorumKey => {
				write!(f, "injected quorum key was not expected key")
			}
			Self::InvalidStateTransition(from, to) => {
				write!(f, "invalid state transition: {:?} -> {:?}", from, to)
			}
			Self::DuplicateApproval => {
				write!(f, "manifest envelope has duplicate approvals")
			}
			Self::DifferentManifest { expected, actual } => {
				write!(f, "different manifest: expected {expected}, got {actual}")
			}
			Self::QosCrypto(msg) => write!(f, "crypto error: {}", msg),
			Self::PoolExpandError => write!(f, "stream pool expand error"),
		}
	}
}
