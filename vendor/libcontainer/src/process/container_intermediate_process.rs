    ...
    let cb: CloneCb = {
        Box::new(|| {
            ...
            match init_process::container_init_process(args, main_sender, init_receiver) {
                Ok(_) => 0,
                Err(e) => {
                    // Preserve the error chain across the process boundary.
                    // Display on several libcontainer errors intentionally only
                    // contains a category such as "failed syscall", which is
                    // insufficient to diagnose a failed enclave boot.
                    let error = format!("{e:?}");
                    tracing::error!("failed to initialize container process: {error}");
                    if let Err(err) = main_sender.exec_failed(error.clone()) {
