...
for app-level attestations. PCR18 commits the manifest hash alone, so it is the
one commitment register that is identical across every instance and every boot
of a deployment; it identifies the application rather than the boot. QOS locks
the full attestable PCR range before publishing the boot files that allow the
pivot app to start.
